...
import com.JH.JhOnlineJudge.domain.order.repository.OrderRepository;
...
import com.JH.JhOnlineJudge.domain.user.exception.InvalidRewardPointsException;
...
public class OrderService {
    ...
    @Transactional(readOnly = true)
    public Slice<Order> searchOrdersByStatusAndText(DeliverySearchFilterDto deliverySearchFilterDto, int page, int size) {

        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "orderAt"));

        return orderRepository.findByStatusAndSearchText(
              deliverySearchFilterDto.getStatus().equals("") ? null : OrderStatus.valueOf(deliverySearchFilterDto.getStatus()),
              deliverySearchFilterDto.getSearch(), pageable);
    }
