...
저는 패턴이 있는 3천만개의 데이터를 임의로 넣어서 자주 등장하는 단어에 대해서는 사실 Like문을 사용한 것과 속도 차이가 많지 않았지만 

<br>

실제 데이터는 규칙적이지 않고 키워드가 매우 다양하기 때문에 오히려 실제 데이터에서 더욱 조회 속도 성능 향상이 체감 될 것이라고 생각합니다. 
